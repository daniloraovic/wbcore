...
class My_Communication_SMPP_SocketTransport extends My_Communication_SMPP
{
	protected $socket;
	protected $hosts;
	protected $persist;
	...
	public function isOpen()
	{
		...
		if ($res === false) throw new My_Communication_SMPP_SocketTransportException('Could not examine socket; '.socket_strerror(socket_last_error()), socket_last_error());
	...
	public function open()
	{
		if (!self::$forceIpv4) {
			...
			if ($socket6 == false) throw new My_Communication_SMPP_SocketTransportException('Could not create socket; '.socket_strerror(socket_last_error()), socket_last_error());
		...
		if (!self::$forceIpv6) {
			...
			if ($socket4 == false) throw new My_Communication_SMPP_SocketTransportException('Could not create socket; '.socket_strerror(socket_last_error()), socket_last_error());
		...
		throw new My_Communication_SMPP_SocketTransportException('Could not connect to any of the specified hosts');
	}
	 ...
	 * Throws My_Communication_SMPP_SocketTransportException if data could not be read.
	 *
	 * @param integer $length
	 * @return mixed
	 * @throws My_Communication_SMPP_SocketTransportException
	 */
	public function read($length)
	{
		...
		if ($d === false) throw new My_Communication_SMPP_SocketTransportException('Could not read '.$length.' bytes from socket; '.socket_strerror(socket_last_error()), socket_last_error());
		if ($d === '') return false;
		return $d;
	}
	 ...
	 * Timeout throws My_Communication_SMPP_SocketTransportException
	 * 
	 * @param integer $length
	 */
	public function readAll($length)
	{
		...
		while ($r < $length) {
			...
			if ($r === false) throw new My_Communication_SMPP_SocketTransportException('Could not read '.$length.' bytes from socket; '.socket_strerror(socket_last_error()), socket_last_error());
			$d .= $buf;
			...
			// check
			if ($res === false) throw new My_Communication_SMPP_SocketTransportException('Could not examine socket; '.socket_strerror(socket_last_error()), socket_last_error());
			if (!empty($e)) throw new My_Communication_SMPP_SocketTransportException('Socket exception while waiting for data; '.socket_strerror(socket_last_error()), socket_last_error());
			if (empty($r)) throw new My_Communication_SMPP_SocketTransportException('Timed out waiting for data on socket');
		}
	}
	...
	public function write($buffer,$length)
	{
		...
		while ($r>0) {
			...
			if ($wrote === false) throw new My_Communication_SMPP_SocketTransportException('Could not write '.$length.' bytes to socket; '.socket_strerror(socket_last_error()), socket_last_error());
			$r -= $wrote;
			if ($r == 0) return;
			...
			// check
			if ($res === false) throw new My_Communication_SMPP_SocketTransportException('Could not examine socket; '.socket_strerror(socket_last_error()), socket_last_error());
			if (!empty($e)) throw new My_Communication_SMPP_SocketTransportException('Socket exception while waiting to write data; '.socket_strerror(socket_last_error()), socket_last_error());
			if (empty($w)) throw new My_Communication_SMPP_SocketTransportException('Timed out waiting to write data on socket');
		}
	}
}