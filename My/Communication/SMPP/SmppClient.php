...
class My_Communication_SMPP_SmppClient extends My_Communication_SMPP
{
	...
	public static $csms_method = My_Communication_SMPP_SmppClient::CSMS_16BIT_TAGS;
	 ...
	 */
	public function __construct(My_Communication_SMPP_SocketTransport $transport, $debugHandler=null)
	{
		// Internal parameters
	...
	public function bindReceiver($login, $pass)
	{
		...
		$response = $this->_bind($login, $pass, My_Communication_SMPP_SMPP::BIND_RECEIVER);
	...
	public function bindTransmitter($login, $pass)
	{
		...
		$response = $this->_bind($login, $pass, My_Communication_SMPP_SMPP::BIND_TRANSMITTER);
	...
	public function close()
	{
		...
		$response=$this->sendCommand(My_Communication_SMPP_SMPP::UNBIND,"");
	 ...
	 * 	message_state would be one of the My_Communication_SMPP_SMPP::STATE_* constants. (SMPP v3.4 section 5.2.28)
	 * 	error_code depends on the telco network, so could be anything.
	 *
	 * @param string $messageid
	 * @param My_Communication_SMPP_SmppAddress $source
	 * @return array
	 */
	public function queryStatus($messageid, My_Communication_SMPP_SmppAddress $source)
	{
		$pduBody = pack('a'.(strlen($messageid)+1).'cca'.(strlen($source->value)+1),$messageid,$source->ton,$source->npi,$source->value);
		$reply = $this->sendCommand(My_Communication_SMPP_SMPP::QUERY_SM, $pduBody);
		if (!$reply || $reply->status != My_Communication_SMPP_SMPP::ESME_ROK) return null;
	...
	{
		$command_id=My_Communication_SMPP_SMPP::DELIVER_SM;
		// Check the queue
		...
		do{
			...
			if($pdu->id==My_Communication_SMPP_SMPP::ENQUIRE_LINK) {
				$response = new My_Communication_SMPP_SmppPdu(My_Communication_SMPP_SMPP::ENQUIRE_LINK_RESP, My_Communication_SMPP_SMPP::ESME_ROK, $pdu->sequence, "\x00");
	 ...
	 * 
	 * @param My_Communication_SMPP_SmppAddress $from
	 * @param My_Communication_SMPP_SmppAddress $to
	 ...
	 */
	public function sendSMS(My_Communication_SMPP_SmppAddress $from, My_Communication_SMPP_SmppAddress $to, $message, $tags=null, $dataCoding=My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT, $priority=0x00, $scheduleDeliveryTime=null, $validityPeriod=null)
	{
		$msg_length = strlen($message);

		if ($msg_length>160 && $dataCoding != My_Communication_SMPP_SMPP::DATA_CODING_UCS2 && $dataCoding != My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT) return false;

		switch ($dataCoding) {
			case My_Communication_SMPP_SMPP::DATA_CODING_UCS2:
				...
				break;
			case My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT:
		...
		if ($msg_length > $singleSmsOctetLimit) {
			$doCsms = true;
			if (!self::$csms_method != My_Communication_SMPP_SmppClient::CSMS_PAYLOAD) {
		...
		if ($doCsms) {
			if (self::$csms_method == My_Communication_SMPP_SmppClient::CSMS_PAYLOAD) {
				$payload = new My_Communication_SMPP_SmppTag(My_Communication_SMPP_SmppTag::MESSAGE_PAYLOAD, $message, $msg_length);
			...
			} else if (self::$csms_method == My_Communication_SMPP_SmppClient::CSMS_8BIT_UDH) {
				$seqnum = 1;
				foreach ($parts as $part) {
					$udh = pack('cccccc',5,0,3,substr($csmsReference,1,1),count($parts),$seqnum);
					$res = $this->submit_sm($from, $to, $udh.$part, $tags, $dataCoding, $priority, $scheduleDeliveryTime, $validityPeriod, (My_Communication_SMPP_SmppClient::$sms_esm_class|0x40));
					$seqnum++;
				}
				return $res;
			} else {
				$sar_msg_ref_num = new My_Communication_SMPP_SmppTag(My_Communication_SMPP_SmppTag::SAR_MSG_REF_NUM, $csmsReference, 2, 'n');
				$sar_total_segments = new My_Communication_SMPP_SmppTag(My_Communication_SMPP_SmppTag::SAR_TOTAL_SEGMENTS, count($parts), 1, 'c');
				$seqnum = 1;
				foreach ($parts as $part) {
					$sartags = array($sar_msg_ref_num, $sar_total_segments, new My_Communication_SMPP_SmppTag(My_Communication_SMPP_SmppTag::SAR_SEGMENT_SEQNUM, $seqnum, 1, 'c'));
	 ...
	 *
	 * @param My_Communication_SMPP_SmppAddress $source
	 * @param My_Communication_SMPP_SmppAddress $destination
	 ...
	 * @return string message id
	 */
	protected function submit_sm(My_Communication_SMPP_SmppAddress $source, My_Communication_SMPP_SmppAddress $destination, $short_message=null, $tags=null, $dataCoding=My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT, $priority=0x00, $scheduleDeliveryTime=null, $validityPeriod=null, $esmClass=null)
	{
		...
		$response=$this->sendCommand(My_Communication_SMPP_SMPP::SUBMIT_SM,$pdu);
	...
	{
		$limit = (My_Communication_SMPP_SmppClient::$csms_method == My_Communication_SMPP_SmppClient::CSMS_8BIT_UDH) ? 255 : 65535;
	 ...
	 * @param integer $dataCoding (optional)
	 */
	protected function splitMessageString($message, $split, $dataCoding=My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT)
	{
		switch ($dataCoding) {
			case My_Communication_SMPP_SMPP::DATA_CODING_DEFAULT:
				$msg_length = strlen($message);
				...
				$parts[] = $part;
				return $parts;
			case My_Communication_SMPP_SMPP::DATA_CODING_UCS2: // UCS2-BE can just use str_split since we send 132 octets per message, which gives a fine split using UCS2
			default:
	 ...
	 * @return My_Communication_SMPP_SmppPdu
	 */
	protected function _bind($login, $pass, $command_id)
	{
		...
		if ($response->status != My_Communication_SMPP_SMPP::ESME_ROK) throw new My_Communication_SMPP_SmppException(My_Communication_SMPP_SMPP::getStatusMessage($response->status), $response->status);
	 ...
	 * @param My_Communication_SMPP_SmppPdu $pdu - received PDU from SMSC.
	 * @return parsed PDU as array.
	 */
	protected function parseSMS(My_Communication_SMPP_SmppPdu $pdu)
	{
		// Check command id
		if($pdu->id != My_Communication_SMPP_SMPP::DELIVER_SM) throw new InvalidArgumentException('PDU is not an received SMS');
		...
		$source = new My_Communication_SMPP_SmppAddress($source_addr,$source_addr_ton,$source_addr_npi);

		$dest_addr_ton = next($ar);
		$dest_addr_npi = next($ar);
		$destination_addr = $this->getString($ar,21);
		$destination = new My_Communication_SMPP_SmppAddress($destination_addr,$dest_addr_ton,$dest_addr_npi);
		...
		if (($esmClass & My_Communication_SMPP_SMPP::ESM_DELIVER_SMSC_RECEIPT) != 0) {
			$sms = new My_Communication_SMPP_SmppDeliveryReceipt($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $service_type, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
			$sms->parseDeliveryReceipt();
		} else {
			$sms = new My_Communication_SMPP_SmppSms($pdu->id, $pdu->status, $pdu->sequence, $pdu->body, $service_type, $source, $destination, $esmClass, $protocolId, $priorityFlag, $registeredDelivery, $dataCoding, $message, $tags);
		}

		if($this->debug) call_user_func($this->debugHandler, "Received sms:\n".print_r($sms,true));

		// Send response of recieving sms
		$response = new My_Communication_SMPP_SmppPdu(My_Communication_SMPP_SMPP::DELIVER_SM_RESP, My_Communication_SMPP_SMPP::ESME_ROK, $pdu->sequence, "\x00");
		$this->sendPDU($response);
		return $sms;
	}
	...
	{
		$response = $this->sendCommand(My_Communication_SMPP_SMPP::ENQUIRE_LINK, null);
		return $response;
	}
	...
	public function respondEnquireLink()
	{
		...
		for($i=0;$i<$ql;$i++) {
			...
			if($pdu->id==My_Communication_SMPP_SMPP::ENQUIRE_LINK) {
				//remove response
				array_splice($this->pdu_queue, $i, 1);
				$this->sendPDU(new My_Communication_SMPP_SmppPdu(My_Communication_SMPP_SMPP::ENQUIRE_LINK_RESP, My_Communication_SMPP_SMPP::ESME_ROK, $pdu->sequence, "\x00"));
			}
		}
	...
	{
		if (!$this->transport->isOpen()) return false;
		$pdu = new My_Communication_SMPP_SmppPdu($id, 0, $this->sequence_number, $pduBody);
		$this->sendPDU($pdu);
		$response=$this->readPDU_resp($this->sequence_number, $pdu->id);
		if ($response === false) throw new My_Communication_SMPP_SmppException('Failed to read reply to command: 0x'.dechex($id));
	 ...
	 * @param My_Communication_SMPP_SmppPdu $pdu
	 */
	protected function sendPDU(My_Communication_SMPP_SmppPdu $pdu)
	{
	...
	protected function readPDU_resp($seq_number, $command_id)
	{
		...
		$command_id=$command_id|My_Communication_SMPP_SMPP::GENERIC_NACK;
		...
		for($i=0;$i<$ql;$i++) {
			$pdu=$this->pdu_queue[$i];
			if (
					($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == My_Communication_SMPP_SMPP::GENERIC_NACK)) ||
					($pdu->sequence == null && $pdu->id == My_Communication_SMPP_SMPP::GENERIC_NACK)
			) {
		...
		do{
			$pdu=$this->readPDU();
			if ($pdu) {
				if ($pdu->sequence == $seq_number && ($pdu->id == $command_id || $pdu->id == My_Communication_SMPP_SMPP::GENERIC_NACK)) return $pdu;
				if ($pdu->sequence == null && $pdu->id == My_Communication_SMPP_SMPP::GENERIC_NACK) return $pdu;
	...
	protected function readPDU()
	{
		...
		if($this->debug) {
			...
			call_user_func($this->debugHandler, " command status  : 0x".dechex($command_status)." ".My_Communication_SMPP_SMPP::getStatusMessage($command_status));
			call_user_func($this->debugHandler, ' sequence number : '.$sequence_number);
		}
		return new My_Communication_SMPP_SmppPdu($command_id, $command_status, $sequence_number, $body);
	}
	...
	protected function parseTag(&$ar)
	{
		...
		$tag = new My_Communication_SMPP_SmppTag($id, $value, $length);
		if ($this->debug) {
