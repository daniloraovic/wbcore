...
class My_Head
{
    ...
    private function _append( $name, $type )
    {
    	...
    	if ( in_array(APPLICATION_ENV, array("production", "staging", "vm_production", "aws_stage", "aws_production")) ) {
    	//if ( APPLICATION_ENV == "production" ) {
    		$this->_head->append( "/{$type}/{$name}.min.{$type}" );
    		return;
    	}
    	
        if ( in_array(APPLICATION_ENV, array("eunet")) ) {
    		$this->_head->append( "/{$type}/{$name}.{$type}" );
    		return;
    	}
